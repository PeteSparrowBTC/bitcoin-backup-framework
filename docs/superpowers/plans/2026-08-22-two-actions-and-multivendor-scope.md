...
- **Prose tells, every one of them a defect here.** No rule of three used as padding. No inflated significance ("stands as", "a testament to", "pivotal", "underscores"). No promotional adjectives ("vibrant", "robust", "seamless"). No participle tails that add nothing ("...ensuring X", "...highlighting Y"). No copula avoidance ("serves as", "boasts", "represents") where "is" or "has" is meant. No negative parallelism ("not just X, but Y") and no clipped tailing negations ("no guessing"). No vague attribution ("experts argue", "industry reports"). No filler ("it is important to note that", "in order to"). No aphorism formulas ("X is the Y of Z"). No signposting ("let's dive in", "here's what you need to know"). No heading followed by a sentence that only restates the heading. No emoji, no curly quotes. Bold lead-in bullets are this document's established convention and stay; do not strip them.
- **No diff-anchored writing.** The document describes what is true, not what changed. A reader arriving fresh never saw the previous version, so "now", "no longer", "previously", "used to", "for a different reason now" and similar spend words telling them that something they never read has moved. Words like "now" are fine when they describe the world (a defect that has been disclosed, a tool that has shipped); they are defects when they describe this document.
